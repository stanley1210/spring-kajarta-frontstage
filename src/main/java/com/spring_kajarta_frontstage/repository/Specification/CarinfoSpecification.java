...
public class CarinfoSpecification {// 這邊別看 是坨大便 他沒有用
    ...
    public static Specification<Carinfo> dynamicSearch(String modelName, Integer productionYear, BigDecimal price,
        ...
        return (root, query, cb) -> {
            ...
            if (suspension != null) {
                predicates.add(cb.like(cb.toString(carinfoJoin.get("suspension")), "%" + suspension + "%"));
            }
            if (door != null) {
                predicates.add(cb.like(cb.toString(carinfoJoin.get("door")), "%" + door + "%"));
            }
            if (passenger != null) {
                predicates.add(cb.like(cb.toString(carinfoJoin.get("passenger")), "%" + passenger + "%"));
            }
            if (rearwheel != null) {
                predicates.add(cb.like(cb.toString(carinfoJoin.get("rear_wheel")), "%" + rearwheel + "%"));
            }
            if (gasoline != null) {
                predicates.add(cb.like(cb.toString(carinfoJoin.get("gasoline")), "%" + gasoline + "%"));
            }
            if (transmission != null) {
                predicates.add(cb.like(cb.toString(carinfoJoin.get("transmission")), "%" + transmission + "%"));
            }
            if (cc != null) {
